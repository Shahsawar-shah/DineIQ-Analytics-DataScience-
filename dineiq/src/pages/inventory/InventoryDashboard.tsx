...
export default function InventoryDashboard() {
  ...
  const forecast = FORECAST_DAILY.filter((f) => f.forecast).reduce((s, f) => s + (f.forecast as number), 0)
    ...
    <>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-3">
        ...
        <ChartCard title="Demand Forecast (7 days)" subtitle="Expected covers, mock model"
          ...
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={FORECAST_DAILY.filter((f) => f.forecast).map((f) => ({ day: f.day, covers: Math.round((f.forecast as number) / 30.7) }))}>
                <defs>
