...
export default function Forecasting() {
  ...
  const stats = useMemo(() => {
    const fc = series.filter((d) => d.forecast !== null)
  ...
  return (
    <>
      ...
      <ChartCard title={`${period} Forecast — Revenue`} subtitle="Solid = history · dashed = forecast · band = 90% confidence range" className="mt-5" height={340}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={series} margin={{ top: 10, right: 12, bottom: 0, left: -8 }}>
            ...
            <YAxis tick={{ fontSize: 10, fill: '#878ba7' }} axisLine={false} tickLine={false} tickFormatter={(v) => `$${v.toLocaleString()}`} />
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          { label: 'Next Period Peak', value: fmtMoney(Math.max(...series.filter((d) => d.forecast).map((d) => d.forecast))), note: 'Highest forecast point' },
